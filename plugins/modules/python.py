...
notes:
  - |
    ...
    * O(state=present)
      ...
      - If only a minor version is specified (for example 3.12), the latest available patch version for that minor release is installed only if no patch version for that minor release is currently installed (including patch versions not managed by C(uv)).
      - RV(python_versions) and RV(python_paths) lengths are always equal to one for this state.
      - Uses C(uv python install) command.
    ...
    * O(state=absent)
      ...
      - Uses C(uv python uninstall) command.
